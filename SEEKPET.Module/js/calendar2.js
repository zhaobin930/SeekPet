 ...
 *
 * Warning!!
 *   Modified by WeiPeng (2008-07-23),Support Time
 */
...
Calendar.prototype.draw = function() {
	...
	for(var i = 0,i2=this.containsTime?5:6; i < i2; i++){
	...
	{
	    _cs[_cs.length] = ' <tr>';
	    _cs[_cs.length] = ' <th colspan="7" align="left">';
	    _cs[_cs.length] = ' <input type="checkbox" name="chk__calendarTime" id="chk__calendarTime" checked="checked" \/>&nbsp;&nbsp;<label for="chk__calendarTime">';
	    _cs[_cs.length] = Calendar.language["time"][this.language];
	    _cs[_cs.length] = ' <\/label></th></tr>';
	}
	...
	if (this.containsTime)
	    this.form.chk__calendarTime.onclick=function() {calendar.containsTime=this.checked;};
	...
	this.form.selectTodayButton.onclick = function () {
		...
		if (calendar.containsTime)
		    calendar.dateControl.value = today.format(calendar.dateTimeFormat);//'yyyy-MM-dd hh:mm:ss');
		else
		    calendar.dateControl.value = today.format(calendar.dateFormat);//'yyyy-MM-dd');
		calendar.hide();
	}
};
				...
				if (calendar.dateControl){
				    if (calendar.containsTime)
				    {
					    ...
					    calendar.dateControl.value = new Date(calendar.date.getFullYear(),
														...
														now.getHours(),now.getMinutes(),now.getSeconds()).format(calendar.dateTimeFormat);//'yyyy-MM-dd hh:mm:ss');
					}
					else
					{
					   calendar.dateControl.value = new Date(calendar.date.getFullYear(),
														calendar.date.getMonth(),this.innerHTML).format(calendar.dateFormat);//'yyyy-MM-dd');
					}
				}
				calendar.hide();
			}
...
Calendar.prototype.show = function (dateControl,containsTime, popuControl) {
	...
	if (containsTime==null||containsTime==undefined)
    ...
    document.onclick=function() {
        ...
        if (sender==null||sender.parentElement==null) 
            calendar.hide();
        else if (sender!=calendar.dateControl&&sender.parentElement.id!='__calendarTable')
            calendar.hide();
                    
        //if (event.srcElement.id!='__calendarTable'&&event.srcElement!=calendar.dateControl)
        //    calendar.hide();
        //document.title="click:"+sender.tagName+",parent:"+sender.parentElement.tagName;
    };
};
