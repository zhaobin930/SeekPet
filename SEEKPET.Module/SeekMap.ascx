<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SeekMap.ascx.cs" Inherits="SEEKPET.Module.SeekMap" %>
<a href="javascript:void(0)" id='gr_zone_ids'>
    <img src="/Portals/0/Skins/seekpet/map.png"></a>
...
<script src="http://api.map.baidu.com/api?ak=E8c48b23ebe765645d441a1a0feaa0f3&v=2.0&services=false"></script>
    ...
    jQuery(function () {
        ...
        if (city == null || city == "") {
            ...
            geolocation.getCurrentPosition(function (r) {
                ...
                //var lon = position.coords.longitude; alert(lat);
                if (this.getStatus() == BMAP_STATUS_SUCCESS) {
                    var mk = new BMap.Marker(r.point);
                    currentLat = r.point.lat;
                    currentLon = r.point.lng;
                    var point = new BMap.Point(currentLon, currentLat);  // 创建坐标点
                    // 根据坐标得到地址描述
                    var myGeo = new BMap.Geocoder();
                    myGeo.getLocation(point, function (result) {
                        var city = result.addressComponents.city; //console.log(city);
                        jQuery('#gr_zone_ids').html(city);
...
((expiredays == null) ? "" : ";expires=" + exdate.toGMTString())+";path=/";
    ...
    //加载城市事件
    jQuery('#gr_zone_ids').click(function () {
        jQuery('.container').show();

    });
    //选择城市 start
    jQuery('.city-list p').click(function () {
        var type = jQuery('.container').data('type');
        jQuery('#zone_ids').html(jQuery(this).html()).attr('data-id', jQuery(this).attr('data-id'));
        jQuery('#gr_zone_ids').html(jQuery(this).html()).attr('data-id', jQuery(this).attr('data-id'));
        //        var url = "/tabid/40/default.aspx?city=" + escape(jQuery('#gr_zone_ids').html());
        //        location.href = url;
        var city = jQuery('#gr_zone_ids').html();
        console.log(city);
        setCookie('city', city, 7);
        jQuery('.container').hide();
        location.href = "/tabid/40/default.aspx?random=" + Math.random();
    });
    jQuery('.letter a').click(function () {
